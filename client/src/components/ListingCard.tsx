...
export default function ListingCard({
 ...
 const rawListingId =
 listingAny.id ?? listingAny.listingId ?? listingAny.listing?.id ?? listingAny.vendorListingId;
 const listingId = rawListingId != null ? String(rawListingId).trim() : "";
 const hasListingId = listingId.length > 0;
 const listingPath = hasListingId ? `/listing/${listingId}` : null;
 ...
 enabled: isAuthenticated && showHeartIcon,
 retry: false,
 staleTime: 30_000,
 });
 ...
 data-testid={`card-listing-${hasListingId ? listingId : "unknown"}`}
 ...
 >
 <Card className="cursor-pointer overflow-hidden rounded-[12px] border-0 bg-white shadow-[0_4px_24px_rgba(74,106,125,0.10)] ">
 ...
 {showHeartIcon && isAuthenticated && hasListingId ? (
 ...
 >
 <h3
 className={`line-clamp-2 ${titleFont === "heading" ? "font-heading" : "font-sans"} font-semibold leading-tight text-[#2a3a42] ${resolvedTitleSizeClass}`}
 >
 {title}
 </h3>
