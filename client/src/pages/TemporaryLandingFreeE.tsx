...
/* ---------------------------------------------------------------------------
   Small shared primitives (restyled for the "Split Momentum" direction)
--------------------------------------------------------------------------- */

function Eyebrow({ children, tone = "coral" }: { children: ReactNode; tone?: "coral" | "cream" }) {
  const color = tone === "cream" ? "text-[#9dd4cc]" : "text-[#e07a6a]";
  return (
    <p className={`mb-3 font-sans text-[0.78rem] font-semibold uppercase tracking-[0.2em] ${color}`}>
      {children}
    </p>
  );
}
...
function PrimaryButton({ onClick, children, className = "" }: { onClick: () => void; children: ReactNode; className?: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-[12px] bg-[#e07a6a] px-6 py-3 font-sans text-[1.05rem] font-semibold text-white transition-colors hover:bg-[#c96959] ${className}`}
    >
      {children}
    </button>
  );
...
/* ---------------------------------------------------------------------------
   Hero "create a listing" demo — a silent, auto-playing replica of the real
   CreateListingWizard (client/src/features/vendor/create-listing). It steps
   through the actual wizard steps (Listing Basics → Perfect For → Booking &
   Pricing → Service Area → Logistics → Photos → Attach Add-ons), mirroring the
   real left step-rail, field styling, and footer, then publishes and loops. No
   audio, cursor, or person — just the tool being used. Product UI labels are
   intentionally hard-coded (not i18n) so the preview matches the actual English
   wizard. Honours prefers-reduced-motion by holding the finished state.
   Interactive: prev/next arrows and the step rail navigate manually (the 3s
   auto-advance keeps running and reschedules after a click), and press-and-hold
   anywhere pauses until release.
--------------------------------------------------------------------------- */


/* ---------------------------------------------------------------------------
   Payouts mock (on a dark ink card for the payments spotlight)
--------------------------------------------------------------------------- */

function PayoutsMock() {
  const { t } = useTranslation();
  const payouts = [
    { label: t(`${NS}.payouts.item1`), amount: "+ $427.50" },
    { label: t(`${NS}.payouts.item2`), amount: "+ $1,140.00" },
    { label: t(`${NS}.payouts.item3`), amount: "+ $902.50" },
  ];
  return (
    <MockShell className="w-full">
      <div className="flex items-center justify-between border-b border-[rgba(74,106,125,0.12)] bg-[#f8fafb] px-5 py-3">
        <span className="font-heading text-[1.05rem] text-[#2a3a42]">{t(`${NS}.payouts.title`)}</span>
        <SampleTag />
      </div>
      <div className="px-5 py-4">
        <p className="font-sans text-[0.72rem] uppercase tracking-wide text-[#9aacb4]">{t(`${NS}.payouts.availableBalance`)}</p>
...
/* ---------------------------------------------------------------------------
   Vendor signup dialog (verbatim from spec; NS-prefixed keys)
--------------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------------
   Page — Direction C "Split Momentum"
--------------------------------------------------------------------------- */
...
export default function TemporaryLandingFreeE() {
  ...
  const benefits = [
    { icon: "+", title: t(`${NS}.benefits.listing.title`), body: t(`${NS}.benefits.listing.body`), tone: "coral" as const },
    { icon: "✓", title: t(`${NS}.benefits.bookings.title`), body: t(`${NS}.benefits.bookings.body`), tone: "mint" as const },
    { icon: "$", title: t(`${NS}.benefits.payments.title`), body: t(`${NS}.benefits.payments.body`), tone: "coral" as const },
    { icon: "✉", title: t(`${NS}.benefits.messaging.title`), body: t(`${NS}.benefits.messaging.body`), tone: "mint" as const },
  ];

  const steps = [
    { title: t(`${NS}.goLive.step1Title`), body: t(`${NS}.goLive.step1Body`) },
    { title: t(`${NS}.goLive.step2Title`), body: t(`${NS}.goLive.step2Body`) },
    { title: t(`${NS}.goLive.step3Title`), body: t(`${NS}.goLive.step3Body`) },
  ];

  return (
    <div className="min-h-screen bg-white">
      {/* Header stack: slim free bar + header, above the split hero */}
      <div className="relative z-40">
        ...
        <div className="bg-[#2a3a42] text-[#f5f0e8]">
          <div className="mx-auto flex max-w-[1320px] flex-wrap items-center justify-center gap-x-3 gap-y-1 px-5 py-2 text-center lg:px-10">
            <span className="font-sans text-[0.92rem] leading-tight">
              <span className="font-semibold text-[#9dd4cc]">{t(`${NS}.bar.badge`)}</span>
              <span className="mx-1.5 text-[rgba(245,240,232,0.4)]">·</span>
              <span className="text-[rgba(245,240,232,0.85)]">{t(`${NS}.bar.text`)}</span>
            </span>
            <button
              type="button"
        ...
        {/* Header */}
        <header className="border-b border-[rgba(74,106,125,0.1)] bg-white">
      ...
      {/* MIRRORED SPLIT HERO — demo left, pitch right (Variation 2: image-left orientation) */}
      <section className="grid lg:grid-cols-2">
        {/* LEFT — bright panel with the demo (order swaps below lg so the pitch leads on mobile) */}
        <div className="relative order-2 overflow-hidden bg-[#f5f0e8] px-6 py-16 lg:order-1 lg:px-0 lg:py-0">
          <div className="flex h-full items-center lg:py-16">
            <div className="mx-auto w-full max-w-[460px] lg:ml-auto lg:mr-10 xl:mr-16">
              <ListingWizardDemo />
            </div>
          </div>
        </div>

        {/* RIGHT — dark ink panel with the pitch */}
        <div className="order-1 flex flex-col justify-center bg-[#2a3a42] px-6 py-16 lg:order-2 lg:px-12 lg:py-28 xl:px-20">
          <div className="mx-auto w-full max-w-[560px]">
            <Eyebrow tone="cream">{t(`${NS}.hero.eyebrow`)}</Eyebrow>
            <h1 className="font-heading text-[clamp(2.7rem,5.5vw,4.4rem)] font-light leading-[1.02] text-[#f5f0e8]">
              <Trans
            ...
            <div className="mt-9 flex flex-wrap items-center gap-4">
              <PrimaryButton onClick={() => handleSignupCta("hero_primary")} className="!px-7 !py-3.5 !text-[1.1rem]">{t(`${NS}.hero.ctaPrimary`)}</PrimaryButton>
              <button
                type="button"
                onClick={openLogin}
                className="rounded-[12px] border-[1.5px] border-[rgba(245,240,232,0.4)] px-7 py-3.5 font-sans text-[1.1rem] font-semibold text-[#f5f0e8] transition-colors hover:bg-[rgba(245,240,232,0.08)]"
              >
                {t(`${NS}.hero.logIn`)}
              </button>
            </div>
            <p className="mt-6 font-sans text-[0.95rem] text-[rgba(245,240,232,0.6)]">{t(`${NS}.hero.trustLine`)}</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#2a3a42]">
        <div className="mx-auto max-w-[1320px] px-5 py-20 text-center lg:px-10 lg:py-24">
          ...
          <div className="mt-9 flex flex-wrap items-center justify-center gap-4">
            <button
              type="button"
              onClick={() => handleSignupCta("closing_cta")}
