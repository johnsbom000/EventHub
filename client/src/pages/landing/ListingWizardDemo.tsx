...
// Mirrors the real wizard's SINGLE_STEPS + STEP_META (titles, subtitles, icons).
const WIZARD_STEPS = [
  { title: "Listing Basics", sub: "Create one listing for one distinct rentable item, set, or style.", Icon: ClipboardList },
  { title: "Perfect For", sub: "Pick the events this listing is perfect for.", Icon: Sparkles },
  { title: "Booking & Pricing", sub: "Set booking behavior, pricing model, and quantity.", Icon: DollarSign },
  { title: "Service Area", sub: "Set your coverage center and radius.", Icon: MapPin },
  { title: "Logistics", sub: "Travel, delivery, and setup options.", Icon: Truck },
  { title: "Photos", sub: "Add at least 3 photos to publish.", Icon: ImageIcon },
  { title: "Attach Add-ons", sub: "Optional upgrades customers can add.", Icon: Paperclip },
] as const;
      ...
      <div className="flex">
        ...
        <div className="relative min-w-0 flex-1">
            ...
            <div className="h-[368px] overflow-hidden rounded-[14px] border border-[rgba(74,106,125,0.16)] bg-white p-4">
              {active === 0 && (
                <div className="space-y-3">
                  <div>
                    <p className="mb-1.5 font-sans text-[0.76rem] font-semibold text-[#2a3a42]">Category</p>
                    <div className="grid grid-cols-4 gap-2">
                      {["Rental", "Service", "Venue", "Caterer"].map((c, i) => (
                        <div
                          key={c}
                  ...
                  <div>
                    <p className="mb-1.5 font-sans text-[0.76rem] font-semibold text-[#2a3a42]">Title</p>
                  ...
                  <div>
                    <p className="mb-1.5 font-sans text-[0.76rem] font-semibold text-[#2a3a42]">What's Included</p>
                    <div className="flex flex-wrap gap-1.5">
                      {["Bounce house", "Blower + stakes", "Setup guide"].map((item) => (
                        <span key={item} className="inline-flex items-center gap-1.5 rounded-lg border border-border px-2.5 py-1 font-sans text-[0.72rem] text-[#2a3a42]">
                  ...
                  <div>
                    <p className="mb-1.5 font-sans text-[0.76rem] font-semibold text-[#2a3a42]">What's Not Included</p>
                    <div className="flex flex-wrap gap-1.5">
                      {["Generator", "Travel over 30 mi"].map((item) => (
                        <span key={item} className="inline-flex items-center gap-1.5 rounded-lg border border-border px-2.5 py-1 font-sans text-[0.72rem] text-[#2a3a42]">
                          <span aria-hidden>•</span>
                          {item}
                          <span className="text-[#9aacb4]">×</span>
              ...
              {active === 1 && (
                <div>
                  <p className="mb-2 font-sans text-[0.76rem] font-semibold text-[#2a3a42]">Perfect for</p>
                  <div className="flex flex-wrap gap-1.5">
                    {[
                      ["💍", "Weddings", true],
                      ["🏢", "Corporate", true],
                      ...
                      ["📸", "Photoshoots", false],
                      ["🎂", "Birthdays", true],
                      ["👰", "Bridal Showers", false],
                      ["🎓", "Graduations", false],
                      ["🎉", "Holiday Parties", true],
                      ["🎵", "Concert", false],
                      ["💐", "Proposal", false],
                      ["🍻", "Bachelor Party", false],
                      ["💃", "Bachelorette Party", false],
                      ["❤️", "Anniversary", false],
                      ["🎈", "Gender Reveal", true],
                      ["👑", "Quinceañera", false],
                      ["🙏", "Baptism", false],
              ...
              {active === 2 && (
                <div className="space-y-3">
                  <div>
                    <p className="mb-1.5 font-sans text-[0.76rem] font-semibold text-[#2a3a42]">Booking Type</p>
                    <div className="flex gap-2">
                      <span className="rounded-md bg-primary px-3 py-1.5 font-sans text-[0.74rem] font-medium text-primary-foreground">Instant Book</span>
                      <span className="rounded-md border border-input px-3 py-1.5 font-sans text-[0.74rem] font-medium text-[#4a6a7d]">Request to Book</span>
                    </div>
                  </div>
                  <div>
                    <p className="mb-1.5 font-sans text-[0.76rem] font-semibold text-[#2a3a42]">Rate per day</p>
                  ...
                  <div className="space-y-2 rounded-xl border border-border bg-muted/30 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <p className="font-sans text-[0.78rem] font-semibold text-[#2a3a42]">Require a security deposit?</p>
                      <span className="flex h-4 w-7 items-center justify-end rounded-full bg-[#4a6a7d] px-0.5">
                        <span className="h-3 w-3 rounded-full bg-white" />
                      </span>
                    </div>
                    <div>
                      <p className="mb-1 font-sans text-[0.72rem] font-medium text-[#4a6a7d]">Deposit Amount</p>
                      <div className="relative max-w-[160px]">
                        <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 font-sans text-[0.82rem] text-[#9aacb4]">$</span>
              ...
              {active === 3 && (
                <div className="space-y-3.5">
                  <div>
                    <p className="mb-1.5 font-sans text-[0.76rem] font-semibold text-[#2a3a42]">Location</p>
                  ...
                  <div>
                    <p className="mb-1.5 font-sans text-[0.76rem] font-semibold text-[#2a3a42]">Service radius</p>
              ...
              {active === 4 && (
                <div className="space-y-2.5">
                  <div className="space-y-2 rounded-xl border border-border p-2.5">
                    <div className="flex items-center justify-between gap-3">
                      <span className="font-sans text-[0.8rem] text-[#2a3a42]">Do you set up?</span>
                      <span className="inline-flex overflow-hidden rounded-lg border border-border font-sans text-[0.72rem] font-medium">
                        <span className="bg-primary px-3 py-1 text-primary-foreground">Yes</span>
                        <span className="border-l border-border bg-background px-3 py-1 text-[#9aacb4]">No</span>
                      </span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span className="font-sans text-[0.8rem] text-[#2a3a42]">Is there a setup fee?</span>
                      <span className="inline-flex overflow-hidden rounded-lg border border-border font-sans text-[0.72rem] font-medium">
                        <span className="bg-primary px-3 py-1 text-primary-foreground">Yes</span>
                        <span className="border-l border-border bg-background px-3 py-1 text-[#9aacb4]">No</span>
                      </span>
                    ...
                    <div>
                      <p className="mb-1 font-sans text-[0.72rem] font-medium text-[#4a6a7d]">Setup fee</p>
                  ...
                  <div>
                    <p className="mb-1.5 font-sans text-[0.76rem] font-semibold text-[#2a3a42]">Cancellation Policy</p>
                    <div className="space-y-1">
                      {[
                        ["Cancel anytime", "Cancel more than 48 hours before the event.", true],
                        ["Cancel within a window", "Cancel up until a set number of hours before.", false],
                        ["No cancellations", "All sales are final once booked.", false],
                      ].map(([label, desc, on]) => (
                        <div
                          key={label as string}
                          className={`rounded-lg border p-2 ${on ? "border-primary bg-primary/5" : "border-border"}`}
                        >
                          <p className="font-sans text-[0.76rem] font-medium text-[#2a3a42]">{label}</p>
                          <p className="font-sans text-[0.7rem] leading-snug text-[#9aacb4]">{desc}</p>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {active === 5 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="font-sans text-[0.76rem] text-[#9aacb4]">4 photos uploaded</span>
                  </div>
              ...
              {active === 6 && (
                <div className="space-y-3">
                  <p className="font-sans text-[0.76rem] font-medium text-[#9aacb4]">Attached add-ons</p>
                  {[
                    ["Setup & styling", "$120 · Qty: 5"],
                    ["Extra hour", "$60/hr · Qty: 10"],
                  ].map(([name, meta]) => (
                    <div key={name} className="flex items-center justify-between gap-3 rounded-lg border border-[rgba(74,106,125,0.16)] px-3 py-2.5">
                      <div className="flex items-center gap-2.5">
                        <Check className="h-4 w-4 shrink-0 text-[#2f7a6b]" />
                        <div>
                          <p className="font-sans text-[0.8rem] font-medium text-[#2a3a42]">{name}</p>
                          <p className="font-sans text-[0.7rem] text-[#9aacb4]">{meta}</p>
                        </div>
                      </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-0.5">
                    <span className="inline-flex items-center gap-1.5 rounded-md border border-input px-3 py-1.5 font-sans text-[0.76rem] font-medium text-[#4a6a7d]">
                      <Plus className="h-3.5 w-3.5" /> Attach existing add-on
                    </span>
                    <span className="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 font-sans text-[0.76rem] font-medium text-primary-foreground">
                      <Plus className="h-3.5 w-3.5" /> Create new add-on
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="flex items-center justify-between gap-2 border-t border-[rgba(74,106,125,0.12)] px-5 py-3">
            <span className="rounded-[10px] border border-[rgba(74,106,125,0.28)] px-4 py-1.5 font-sans text-[0.8rem] font-medium text-[#4a6a7d]">Back</span>
            <div className="flex items-center gap-2.5">
              <span className="font-sans text-[0.78rem] text-[#9aacb4]">Draft saved</span>
              <span
          ...
          {live && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-2.5 bg-white/95 px-6 text-center">
              ...
              <p className="font-heading text-[1.4rem] leading-tight text-[#2a3a42]">Your listing is live!</p>
